 ...
 *
 */
class Default_Lib_Image {
	protected $_noimagePath					= 'images/missingimage.jpg';
	 ...
	 * предполагаем что setSize и setImage уже были вызван
	 *
	 */
	public function resize()
	{
		...
		$this->reset();//сбрасываем все к хуя
	 ...
	 *
	 * @todo написана на коленке, надо будет за ней следить внимательн
	 * @param int $x
	 * @param int $y
	 * @param width $width
	 * @param height $height
	...
	public function output()
	{
		switch ($this->getDestinationFileType()) {
			...
			case 'jpg':
				imagejpeg($this->getDestinationResource(), $this->_destinationImagePath, 80);
				break;
	...
	private function _setDestinationImage()
	{
		if (!$this->_manualDestinationImagePath) {
			...
			$params               = $this->_destinationImageHeight . $this->_destinationImageWidth . $this->_heightCrop . filemtime($this->_resourceImagePath);
